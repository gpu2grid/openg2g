...
Style from `openg2g-paper/figures/paper_plots.mplstyle` (Arial 9 pt,
...
_MPLSTYLE_PATH = Path("/Users/jaywonchung/workspace/openg2g-paper/figures/paper_plots.mplstyle")
...
EXP_LABELS = {
    ...
    "itl_deadline": "E. DC capacity under SLO",
}
...
def _line_handle(color: str, label: str) -> Line2D:
    ...
    return Line2D(
        [0], [0], color=color, marker="o", markersize=5.5,
        markeredgecolor="black", markeredgewidth=0.4, lw=1.6, label=label,
    )
...
def _combo_handle(color: str, label: str):
    ...
    line = Line2D([0], [0], color=color, marker="o", markersize=5.5,
                  markeredgecolor="black", markeredgewidth=0.4, lw=1.6)
...
def _legend_above(ax, handles, *, ncol: int) -> None:
    ...
    ax.legend(
        ...
        frameon=False, handlelength=1.3, columnspacing=1.1,
        handletextpad=0.5, borderaxespad=0,
    )


def _save_legend(handles, stem: Path, *, ncol: int, width: float,
    ...
    if tuple_handles:
        ax.legend(
            ...
            ncol=ncol, loc="center", frameon=False, fontsize=legend_fontsize,
            handlelength=2.6, columnspacing=1.2, handletextpad=0.5,
            handler_map={tuple: HandlerTuple(ndivide=None, pad=0.2)},
        )
    else:
        ax.legend(
            handles=plain_handles, ncol=ncol, loc="center", frameon=False,
...
def _grouped_bars(
    ax, base_vals, ofo_vals, colors, *,
    base_errs=None, ofo_errs=None,
    ylog: bool = True, ylabel: str = "Integral violation\n(pu·s)",
    ...
    for i, color in enumerate(colors):
        ...
        ax.bar(xs[i] - offset, base_vals[i], bar_width,
               color=color, hatch="///", edgecolor="white", linewidth=0.6,
               yerr=be, capsize=1.8, error_kw={"lw": 0.7})
        ax.bar(xs[i] + offset, ofo_vals[i], bar_width,
               color=color, edgecolor="white", linewidth=0.6,
               yerr=oe, capsize=1.8, error_kw={"lw": 0.7})
    ax.set_xticks([])
...
def _plot_pareto(
    ax, entries, logistic_models, color_map, label_map,
    *, marker_size: int = 5.5,
) -> None:
    ...
    for entry in entries:
        ...
        ax.plot(p_mw, t_m, "-o", color=color, markersize=marker_size,
                markeredgecolor="black", markeredgewidth=0.4,
                label=label, alpha=0.95, lw=1.6)


def _pair_bars(
    ax, x, base_vals, ofo_vals, *, ylog: bool, ylabel: str,
    base_errs=None, ofo_errs=None, tick_labels=None, rotation: float = 25.0,
) -> None:
    width = 0.38
    ax.bar(x - width / 2, base_vals, width, yerr=base_errs,
           color=COLORS["baseline"], label="Baseline", capsize=2.5)
    ax.bar(x + width / 2, ofo_vals, width, yerr=ofo_errs,
           color=COLORS["ofo"], label="OFO", capsize=2.5)
    ax.set_xticks(x)
...
def plot_model_size(df, out, logistic_models, *, suffix: str = "") -> None:
    agg = _aggregate(df)
    ...
    fig, ax = _make_panel()
    _plot_pareto(ax, entries, logistic_models,
                 color_map=color_map,
                 label_map={v: _pretty(v) for v in variants})
    ...
    _save_legend(handles, out / f"model_size_legend{suffix}",
                 ncol=len(variants), width=4.6)


# ──────────────────────────────────────────────────────────────────────
# Experiment B — ITL deadline


def plot_itl_deadline(df, out, logistic_models, *, suffix: str = "") -> None:
    ...
    piv = (df.groupby(["deadline_ms", "mode", "endpoint"])["num_replicas"]
             .first().unstack("endpoint").reset_index())
    ...
    ax.fill_between(xs, base["r_min"], base["r_max"],
                    color=COLORS["baseline"], alpha=0.45,
                    label="Baseline feasible")
    # OFO's additional unlocked region — between baseline's R_max and OFO's R_max.
    ax.fill_between(xs, base["r_max"].to_numpy(), ofo["r_max"].to_numpy(),
                    color=COLORS["ofo"], alpha=0.50,
                    label="Unlocked by OFO")
    ...
    _save(fig, out / f"itl_deadline_a{suffix}")
...
def plot_parallelism(df, out, logistic_models, *, suffix: str = "") -> None:
    ...
    for pair in pairs:
        ...
        variants = (
            sub[["variant", "parallelism_label"]]
            .drop_duplicates().sort_values("variant")["variant"].tolist()
        )
        ...
        color_map = {labels[i]: (COLORS["h100"] if i == 0 else COLORS["b200"])
        ...
        entries = []
        for vv, lbl in zip(variants, labels):
        ...
        _plot_pareto(ax, entries, logistic_models,
                     color_map=color_map,
                     label_map={vv: lbl for vv, lbl in zip(variants, labels)})
        _apply_pareto_axes(ax)
        ...
        _save_legend(handles, out / f"{stem}_legend{suffix}",
                     ncol=len(labels), width=2.3)
...
def plot_hardware(df, out, logistic_models, *, suffix: str = "") -> None:
    ...
    # Colors by model (matching Fig 2 style).
    palette = plt.get_cmap("tab10").colors
    pair_colors = {p: palette[i % len(palette)] for i, p in enumerate(pairs)}
    pair_labels = [_pretty(p) for p in pairs]
    ...
    for pi, pair in enumerate(pairs):
        ...
        for mi, mode in enumerate([BASELINE_MODE, OFO_MODE]):
            hatch = "///" if mode == BASELINE_MODE else ""
            offset = (pi - (n - 1) / 2) * 2 * bar_width + (mi - 0.5) * bar_width
    ...
    for pair in pairs:
        ...
        for hw in hardware_order:
            ...
            if len(batches) == 0:
                continue
            if hw == "H100":
                ax.plot(p_mw, t_m, "-o", color=color, markersize=5.5,
                        markerfacecolor="white", markeredgecolor=color,
                        markeredgewidth=1.3, lw=1.6, alpha=0.95)
            else:
                ax.plot(p_mw, t_m, "-o", color=color, markersize=5.5,
                        markeredgecolor="black", markeredgewidth=0.4,
                        lw=1.6, alpha=0.95)
    ...
    hw_key = [
        Line2D([0], [0], marker="o", color="#555", markerfacecolor="white",
               markeredgecolor="#555", markeredgewidth=1.3, lw=1.0, label="H100"),
        Line2D([0], [0], marker="o", color="#555", markerfacecolor="#555",
               markeredgecolor="black", markeredgewidth=0.4, lw=1.0, label="B200"),
    ]
    ax.legend(handles=hw_key, loc="upper left", frameon=False,
    ...
    _save_legend(handles, out / f"hardware_legend{suffix}",
                 ncol=len(pairs), width=2.5)
...
def plot_precision(df, out, logistic_models, *, suffix: str = "") -> None:
    ...
    agg = _aggregate(df).merge(
        df[["variant", "precision_label"]].drop_duplicates(), on="variant", how="left",
    )
    ...
    _display_prec = lambda l: "BF16" if l == "bf16" else l
    handles = [_combo_handle(color_map[lbl], _display_prec(lbl)) for lbl in sorted(set(labels))]
    _save_legend(handles, out / f"precision_legend{suffix}",
                 ncol=len(set(labels)), width=1.8)
...
def main(
    ...
    suffix: str = "",
    paper_figure_dir: Path | None = None,
) -> None:
    ...
    Args:
        ...
        suffix: GPU suffix appended to every filename (e.g., `_b200`).
        paper_figure_dir: If set, copy every generated PDF into this
            directory as well. Intended for syncing into the paper repo
            (`openg2g-paper/figures`).
    """
    import shutil
    ...
    for name in ("model_size", "itl_deadline", "parallelism", "hardware", "precision"):
        path = indir / f"{name}{suffix}.csv"
        if not path.exists():
            logger.warning("Missing %s — skipping", path)
            continue
        dfs[name] = pd.read_csv(path)
        logger.info("Loaded %s: %d rows", path.name, len(dfs[name]))

    if "model_size" in dfs:
        plot_model_size(dfs["model_size"], outdir, lm, suffix=suffix)
    if "itl_deadline" in dfs:
        plot_itl_deadline(dfs["itl_deadline"], outdir, lm, suffix=suffix)
    if "parallelism" in dfs:
        plot_parallelism(dfs["parallelism"], outdir, lm, suffix=suffix)
    if "hardware" in dfs:
        plot_hardware(dfs["hardware"], outdir, lm, suffix=suffix)
    if "precision" in dfs:
        plot_precision(dfs["precision"], outdir, lm, suffix=suffix)
    ...
    if paper_figure_dir is not None:
        ...
        copied = 0
        for pdf in outdir.glob(f"*{suffix}.pdf"):
            shutil.copy2(pdf, paper_figure_dir / pdf.name)
            copied += 1
        logger.info("Synced %d PDFs into %s", copied, paper_figure_dir)
