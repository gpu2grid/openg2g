"""Tests for LogisticParams: eval, derivative correctness, and numerical gradient check."""

from __future__ import annotations

import math

from openg2g.models.logistic import LogisticParams


def test_eval_at_midpoint():
    """At x = x0, sigmoid = 0.5, so y = b0 + L/2."""
    lp = LogisticParams(L=100.0, x0=5.0, k=1.0, b0=50.0)
    assert abs(lp.eval_x(5.0) - 100.0) < 1e-10


def test_eval_at_extremes():
    """Far from x0, sigmoid approaches 0 or 1."""
    lp = LogisticParams(L=100.0, x0=5.0, k=2.0, b0=10.0)
...
def test_eval_batch():
    ...
    lp = LogisticParams(L=200.0, x0=7.0, k=0.5, b0=30.0)
...
def test_derivative_numerical_gradient():
    ...
    lp = LogisticParams(L=150.0, x0=6.0, k=1.5, b0=20.0)
...
def test_derivative_sign():
    ...
    lp = LogisticParams(L=100.0, x0=5.0, k=1.0, b0=0.0)
    for x in [3.0, 5.0, 7.0]:
        assert lp.deriv_wrt_x(x) > 0

    # Negative L means derivative is negative
    lp_neg = LogisticParams(L=-100.0, x0=5.0, k=1.0, b0=200.0)
    for x in [3.0, 5.0, 7.0]:
...
def test_derivative_peak_at_midpoint():
    ...
    lp = LogisticParams(L=100.0, x0=5.0, k=2.0, b0=0.0)
