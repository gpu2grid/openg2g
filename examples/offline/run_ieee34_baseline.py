"""IEEE 34-bus baseline simulation with multi-DC, PV, and time-varying loads.

Two DC sites (upstream@850, downstream@834) with PV generation and industrial
loads creating time-varying voltage stress. Fixed taps, no OFO control.

Usage:
    python run_ieee34_baseline.py --config config_ieee34.json
"""
...
logger = logging.getLogger("run_ieee34_baseline")
...
# Multi-site profiles with per-site variation (cloud events, load surges).
...
def main(*, config_path: Path) -> None:
    ...
    save_dir = (Path(__file__).resolve().parent / "outputs" / "ieee34" / "baseline_no-tap")
    ...
    for site_id, site_cfg in config.dc_sites.items():
        ...
        logger.info("  Site '%s' @ bus %s: %s, base=%.1f kW/ph",
    ...
    logger.info("Running IEEE 34-bus baseline (%d DC sites, %d PV, %d ext loads, no OFO)...",
    ...
    plot_allbus_voltages_per_phase(
        ...
        title_template="IEEE 34 Baseline — Voltage (Phase {label})",
    )
