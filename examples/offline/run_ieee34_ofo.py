"""IEEE 34-bus OFO simulation with multi-DC, PV, and time-varying loads.

Two DC sites (upstream@850, downstream@834) with PV generation and industrial
loads creating time-varying voltage stress. OFO batch-size controllers at both
sites, fixed taps (no tap changes).

Also runs baseline for comparison, producing a comparison CSV and plots.

Usage:
    python run_ieee34_ofo.py --config config_ieee34.json
"""
...
logger = logging.getLogger("run_ieee34_ofo")
...
class IEEE34Config(BaseModel):
    ...
    ieee_case_dir: Path
    dss_master_file: str = "ieee34Mod1_halfline.dss"
    mlenergy_data_dir: Path | None = None

    source_pu: float = 1.09
...
def run_mode(
    ...
    *,
    ...
    # Add tap schedule controller (before OFO controllers, but after they're appended
    # we insert at position 0 so taps are applied first)
    sched = tap_schedule if tap_schedule is not None else TapSchedule(())
    controllers.insert(0, TapScheduleController(schedule=sched, dt_s=dt_ctrl))
    ...
    plot_allbus_voltages_per_phase(
        ...
        title_template=f"IEEE 34 {mode.upper()} — Voltage (Phase {{label}})",
    )
...
def main(*, config_path: Path, mode: str = "no-tap") -> None:
    ...
    config = IEEE34Config.model_validate_json(config_path.read_bytes())
    sim = config.simulation
    ...
    save_dir = (Path(__file__).resolve().parent / "outputs" / "ieee34")
    ...
    # Plot event profiles
    _plot_event_profiles(config.pv_systems, config.time_varying_loads,
                         [save_dir / "baseline_no-tap", save_dir / "ofo"])
    ...
    # Check for tap schedule (IEEE34Config may not have tap_schedule field)
    has_tap_schedule = hasattr(config, "tap_schedule") and config.tap_schedule is not None and len(config.tap_schedule) > 0
    tap_sched = None  # IEEE 34 currently has no tap schedule
    ...
    for run_mode_name, folder, sched in cases:
        stats = run_mode(run_mode_name, **shared, tap_schedule=sched, folder_name=folder)
        results[folder] = stats

    b = results.get("baseline_no-tap") or next(iter(results.values()))
    o = results.get("ofo") or next(v for k, v in results.items() if "ofo" in k)
    ...
    # Comparison CSV
    csv_path = save_dir / "results_ieee34_comparison.csv"
    ...
    # Comparison bar chart — save to ofo folder
    ...
    summary_lines = [
        "",
        "=" * 90,
        "IEEE 34-Bus Comparison — Baseline vs OFO (60 min, fixed taps)",
        "=" * 90,
        f"{'Mode':<12s} {'Viol(s)':>10s} {'Vmin':>10s} {'Vmax':>10s} {'Integral':>14s}",
        "-" * 90,
        f"{'baseline':<12s} {b.violation_time_s:>10.1f} {b.worst_vmin:>10.4f} {b.worst_vmax:>10.4f} {b.integral_violation_pu_s:>14.4f}",
        f"{'ofo':<12s} {o.violation_time_s:>10.1f} {o.worst_vmin:>10.4f} {o.worst_vmax:>10.4f} {o.integral_violation_pu_s:>14.4f}",
        "-" * 90,
        f"OFO improvement:  violation time {viol_red},  violation integral {integ_red}",
        f"Outputs: {save_dir}",
    ]
...
def _plot_comparison(results: dict[str, VoltageStats], save_dir: Path) -> None:
    ...
    fig.suptitle("IEEE 34-Bus: Baseline vs OFO", fontsize=14)
    fig.tight_layout()
    fig.savefig(save_dir / "results_ieee34_basic_comparison.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    logger.info("Comparison plot: %s", save_dir / "results_ieee34_basic_comparison.png")
